            ...
            if (response.data) {
                const sortedData = response.data
                    ...
                    .map((item) => {
                        ...
                        const time = (item.hours + item.minutes / 60).toFixed(
                            2
                        );
                        const silverPerHour =
                            Math.floor(item.silver_total / time / 1000000) +
                            ' M';
                ...
                const sessionsByLocation = response.data.reduce(
                    (acc, session) => {
                ...
                setChartsData(
                    Object.keys(sessionsByLocation).map((locationName) => {
                        const locationSessions =
                            sessionsByLocation[locationName];
                        const totalSilver =
                        ...
                        const totalHours = locationSessions.reduce(
                            (sum, session) =>
                                sum + session.hours + session.minutes / 60,
                            0
                        );
